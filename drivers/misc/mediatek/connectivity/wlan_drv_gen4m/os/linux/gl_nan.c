 ...
 */
/*----------------------------------------------------------------------------*/
static int nanInit(struct net_device *prDev)
{
	if (!prDev)
		return -ENXIO;
 ...
 */
/*----------------------------------------------------------------------------*/
static void nanUninit(IN struct net_device *prDev)
{
}
...
unsigned char nanAllocInfo(IN struct GLUE_INFO *prGlueInfo, uint8_t ucRoleIdx)
{
	...
	if (prGlueInfo->aprNANDevInfo[ucRoleIdx] == NULL) {
		...
		for (ucIdx = 0; ucIdx < NAN_BSS_INDEX_NUM; ucIdx++) {
			prWifiVar->aprNanSpecificBssInfo[ucIdx] = kalMemAlloc(
	...
	for (ucIdx = 0; ucIdx < NAN_BSS_INDEX_NUM; ucIdx++) {
		if (prWifiVar->aprNanSpecificBssInfo[ucIdx]) {
			kalMemFree(prWifiVar->aprNanSpecificBssInfo[ucIdx],
				   VIR_MEM_TYPE,
				   sizeof(_NAN_SPECIFIC_BSS_INFO_T));
			     ...
			     unsigned char fgIsRtnlLockAcquired)
{
	...
	netif_carrier_off(prGlueInfo->aprNANDevInfo[eRole]->prDevHandler);
	netif_tx_stop_all_queues(
		prGlueInfo->aprNANDevInfo[eRole]->prDevHandler);

	/* register for net device */
	if (register_netdev(prGlueInfo->aprNANDevInfo[eRole]->prDevHandler) <
	    0) {
		DBGLOG(INIT, WARN, "unable to register netdevice for nan\n");
		/* trunk doesn't do free_netdev here */
		free_netdev(prGlueInfo->aprNANDevInfo[eRole]->prDevHandler);
		...
		dev_change_flags(
			prGlueInfo->aprNANDevInfo[eRole]->prDevHandler,
			prGlueInfo->aprNANDevInfo[eRole]->prDevHandler->flags |
				IFF_UP
			       ...
			       unsigned char fgIsRtnlLockAcquired)
{
...
#if CFG_ENABLE_UNIFY_WIPHY
	{
		...
		for (u4Idx = 0; u4Idx < KAL_AIS_NUM; u4Idx++) {
			if (gprWdev[u4Idx] &&
			    (prNANInfo->prDevHandler == gprWdev[u4Idx]->netdev))
				return FALSE;
		}
	}
#endif
	       ...
	       struct net_device *prNanDev, int u4Idx)
{
	...
	if (prAdapter->fgIsSupportCsumOffload) {
		prNanDev->features = NETIF_F_IP_CSUM | NETIF_F_IPV6_CSUM |
				     NETIF_F_RXCSUM;
	}
 ...
 */
/*----------------------------------------------------------------------------*/
void mtk_nan_wext_set_Multicastlist(struct GLUE_INFO *prGlueInfo)
{
	...
	if (u4PacketFilter & PARAM_PACKET_FILTER_MULTICAST) {
		...
		netdev_for_each_mc_addr(ha, prDev) {
			if (i < MAX_NUM_GROUP_ADDR) {
				...
				DBGLOG(NAN, INFO,
				       "SEt Multicast Address List " MACSTR
				       "\n",
				       MAC2STR(GET_ADDR(ha)));
				i++;
			}
 ...
 */
/*----------------------------------------------------------------------------*/
unsigned char glRegisterNAN(struct GLUE_INFO *prGlueInfo, const char *prDevName)
{
	struct ADAPTER *prAdapter = NULL;
	uint8_t rMacAddr[6];
...
/* allocate netdev */
	...
	prNanDev = alloc_netdev_mq(sizeof(struct NETDEV_PRIVATE_GLUE_INFO),
				   prSetDevName, NET_NAME_PREDICTABLE,
				   ether_setup, CFG_MAX_TXQ_NUM);
#else
	prNanDev = alloc_netdev_mq(sizeof(struct NETDEV_PRIVATE_GLUE_INFO),
				   prSetDevName, ether_setup, CFG_MAX_TXQ_NUM);
#endif
	if (!prNanDev) {
	...
	ether_addr_copy(prNanDev->perm_addr, prNanDev->dev_addr);
 ...
 */
/*----------------------------------------------------------------------------*/
struct net_device_stats *nanGetStats(IN struct net_device *prDev)
{
...
static void nanSetMulticastList(IN struct net_device *prDev)
{
	...
	if (g_aprNanMultiDev[ucRoleIdx].fgBMCFilterSet == FALSE) {
		 ...
		 * finish current job
		 */
		set_bit(GLUE_FLAG_NAN_MULTICAST_BIT, &prGlueInfo->ulFlag);
		/* wake up main thread */
 ...
 */
/*----------------------------------------------------------------------------*/
netdev_tx_t nanHardStartXmit(IN struct sk_buff *prSkb,
			     IN struct net_device *prDev)
{
	...
	if (kalQoSFrameClassifierAndPacketInfo(prGlueInfo, prSkb,
					       &prTxPktInfo)) {
		if (IS_BMCAST_MAC_ADDR(prTxPktInfo.aucEthDestAddr)) {
			DBGLOG(NAN, LOUD, "TX with DA = BMCAST\n");
		} else {
			prStaRec =
				nanGetStaRecByNDI(prGlueInfo->prAdapter,
...
int nanDoIOCTL(struct net_device *prDev, struct ifreq *prIfReq, int i4Cmd)
{
	struct GLUE_INFO *prGlueInfo = NULL;
	int ret = 0;
