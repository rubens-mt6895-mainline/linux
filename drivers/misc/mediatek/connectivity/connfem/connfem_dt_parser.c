				     ...
				     unsigned int *nbits_out)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		/* Retrieve GPIO descriptor */
		desc = fwnode_gpiod_get_index(of_fwnode_handle(np), NULL, i,
					      GPIOD_IN, "connfem-hwid");

		/* Abort if it's not valid */
		if (IS_ERR(desc)) {
			pr_info("[WARN] Invalid '%s' property: idx(%d)err(%ld)",
				CFM_DT_NODE_HWID,
				i, PTR_ERR(desc));
			hwid = 0;
			return -EINVAL;
		...
		/* Read GPIO PIN */
		gpio_value = gpiod_get_value(desc);
		hwid |= ((gpio_value & 0x1) << i);
		pr_info("%s[%d]: GPIO=%d, hwid(%u)",
			CFM_DT_NODE_HWID, i,
			gpio_value, hwid);
		gpiod_put(desc);
