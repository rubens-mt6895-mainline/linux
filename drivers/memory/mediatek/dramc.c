...
static int dramc_probe(struct platform_device *pdev)
{
	...
	/* Mainline adaptation: only support_ch_cnt drives the register
	 * mapping; everything else is informational and optional so the
	 * driver probes on boards whose bootloaders publish none of the
	 * per-project DRAM description properties.
	 */
	if (of_property_read_u32(dramc_node,
	    "dram_type", &(dramc_dev_ptr->dram_type)))
		dramc_dev_ptr->dram_type = 0;
	...
	if (of_property_read_u32(dramc_node,
		...
		dramc_dev_ptr->ch_cnt = dramc_dev_ptr->support_ch_cnt;

	if (of_property_read_u32(dramc_node, "rk_cnt", &dramc_dev_ptr->rk_cnt))
		dramc_dev_ptr->rk_cnt = 0;

	if (of_property_read_u32(dramc_node, "mr_cnt", &dramc_dev_ptr->mr_cnt))
		dramc_dev_ptr->mr_cnt = 0;

	if (of_property_read_u32(dramc_node, "freq_cnt",
				 &dramc_dev_ptr->freq_cnt))
		dramc_dev_ptr->freq_cnt = 0;
	...
	if (dramc_dev_ptr->rk_cnt) {
		size = sizeof(unsigned int) * dramc_dev_ptr->rk_cnt;
		dramc_dev_ptr->rk_size =
			devm_kmalloc(&pdev->dev, size, GFP_KERNEL);
		if (!(dramc_dev_ptr->rk_size))
			return -ENOMEM;
		if (of_property_read_u32_array(dramc_node,
			"rk_size", dramc_dev_ptr->rk_size,
	...
	if (dramc_dev_ptr->mr_cnt) {
		...
		dramc_dev_ptr->mr_info_ptr =
			devm_kmalloc(&pdev->dev, size, GFP_KERNEL);
		if (!(dramc_dev_ptr->mr_info_ptr))
			return -ENOMEM;
		if (of_property_read_u32_array(dramc_node, "mr",
			(unsigned int *)dramc_dev_ptr->mr_info_ptr,
			size >> 2)) {
			dramc_dev_ptr->mr_cnt = 0;
			dramc_dev_ptr->mr_info_ptr = NULL;
		}
	}
	...
	if (dramc_dev_ptr->freq_cnt) {
		...
		if (!(dramc_dev_ptr->freq_step))
			return -ENOMEM;
		if (of_property_read_u32_array(dramc_node, "freq_step",
