...
/* linux/of_gpio.h was removed from newer mainline kernels; the GPIO helpers
 * this driver uses (of_get_named_gpio etc.) now live in of.h/gpio/consumer.h */
...
static bool lcm_panel_is_p0(void)
{
...
static int lcm_get_modes(struct drm_panel *panel, struct drm_connector *connector)
{
	...
	/*
	 * Only the 1440x3200 (WQHD) modes are exposed: rubens (Redmi K50) uses
	 * the WQHD l11a panel.  The 1080x2400 (FHDP) modes belong to the FHD
	 * variant of this panel family and, when present, DRM picked one of
	 * them instead of the preferred WQHD mode, which left the console at
	 * 135x153 on a 1440x3200 screen.
	 */
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	/* MIPI_DSI_MODE_EOT_PACKET no longer exists in newer kernels: EOT packets
	 * are sent by default, so the old "enable EOT" flag is simply dropped */
	...
	/* Newer kernels dropped drm_panel_init(), so initialise the struct the
	 * same way it used to: drm_panel_add()/of_drm_find_panel() walk the
	 * panel list and dereference panel->dev->of_node, so leaving dev NULL
	 * makes mtk_dsi's probe crash on a NULL dereference. */
