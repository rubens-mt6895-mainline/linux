	...
	if (strncmp(opt, "regw:", 5) == 0) {
		...
		for (i = 0; i < ARRAY_SIZE(gdrm_disp1_reg_range); i++) {
			if (addr >= gdrm_disp1_reg_range[i].reg_base &&
			    addr < gdrm_disp1_reg_range[i].reg_base +
					    0x1000UL) {
				pr_info("%8s Read register 0x%08lX: 0x%08X\n",
		...
		pr_info("Set the register addr for Auto-test\n");
		...
		for (i = 0; i < ARRAY_SIZE(gdrm_disp1_reg_range); i++) {
			...
			for (j = gdrm_disp1_reg_range[i].offset[0];
			     ...
			     j += 16UL)
				pr_info("%8s 0x%08lX: %08X %08X %08X %08X\n",
		...
		if (dbgfs_alpha) {
			pr_info("set src alpha to src alpha\n");
			dbgfs_alpha = false;
		} else {
			pr_info("set src alpha to ONE\n");
			dbgfs_alpha = true;
		}
...
void mtk_drm_debugfs_init(struct drm_device *dev, struct mtk_drm_private *priv)
{
	...
	for (i = 0; i < priv->data->main_path_data->path_len[DDP_MAJOR][0] - 1;
		...
		if (ret < 0)
			pr_info("comp_node[%d] map address fail\n", i);
	...
	if (ret < 0)
		pr_info("mutex_node map address fail\n");
