...
/*
 * rubens: the L11a does not answer at all at the stock 12 MHz (writes
 * complete, reads come back empty and the chip-id read yields 0x00/0x68
 * instead of 0x36/0x40), so start from 1 MHz to tell a signal-integrity /
 * sampling-point problem apart from a chip that is simply not running.
 */
