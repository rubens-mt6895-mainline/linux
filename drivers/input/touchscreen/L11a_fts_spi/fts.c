...
static void fts_resume_work(struct work_struct *work)
{
	...
	/* xiaomi_touch_set_suspend_state(XIAOMI_TOUCH_RESUME); */ /* rubens: vendor Xiaomi touch feature disabled */
}
...
/*
 * rubens: linux/of_gpio.h - and with it of_get_named_gpio_flags() - was removed
 * from the kernel.  Resolve the DT specifier to a legacy GPIO number through a
 * temporary descriptor, then release it again: the rest of this driver still
 * drives these lines with gpio_request()/gpio_set_value()/gpio_to_irq(), which
 * would fail with -EBUSY if we kept the descriptor claimed.
 * The device tree uses the conventional "irq-gpios" / "reset-gpios" names.
 */
static int fts_get_named_gpio(struct device *dev, struct device_node *np,
			      const char *con_id)
{
	struct gpio_desc *desc;
	int gpio;

	desc = devm_fwnode_gpiod_get_index(dev, of_fwnode_handle(np), con_id, 0,
					   GPIOD_ASIS, "fts_touch");
	if (IS_ERR(desc)) {
...
static int parse_dt(struct device *dev, struct fts_hw_platform_data *bdata)
{
	...
	bdata->irq_gpio = fts_get_named_gpio(dev, np, "irq");
	if (bdata->irq_gpio < 0)
	...
	if (of_property_read_bool(np, "fts,reset-gpio-enable")) {
		bdata->reset_gpio = fts_get_named_gpio(dev, np, "reset");
		if (bdata->reset_gpio < 0)
	...
	/*
	 * rubens: the vendor parses this one inside parse_gamemode_dt(), which
	 * only runs under FTS_XIAOMI_TOUCHFEATURE.  With that macro disabled the
	 * flag stayed 0 and fts_input_report() decoded the touch report with the
	 * legacy 12-bit packed layout even though the L11a is a protocol-6 IC
	 * that sends full 16-bit X/Y - the pointer then only covered the
	 * top-left corner of the panel.  Parse it here, where it is always run.
	 */
	if (of_find_property(np, "fts,support-super-resolution", NULL)) {
