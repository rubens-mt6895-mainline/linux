...
int tfa_irq_clear(struct tfa_device *tfa, enum tfa9912_irq bit)
{
	...
	if (bit == tfa9912_irq_all) {
		...
		for (reg = TFA98XX_INTERRUPT_IN_REG1; reg < TFA98XX_INTERRUPT_IN_REG1 + 3; reg++)
			tfa_dsp_reg_write(tfa, reg, 0xffff); /* all bits */
	}
	else if (bit < tfa9912_irq_max) {
		reg = (unsigned char)(TFA98XX_INTERRUPT_IN_REG1 + (bit >> 4));
		tfa_dsp_reg_write(tfa, reg, 1 << (bit & 0x0f)); /* only this bit */
	}
	else
		return -1;
...
int tfa_irq_get(struct tfa_device *tfa, enum tfa9912_irq bit)
{
	...
	if (bit < tfa9912_irq_max) {
		...
		mask = 1 << (bit & 0x0f);
		tfa_dsp_reg_read(tfa, (unsigned char)reg, &value);
	}
	else
		return -1;
...
int tfa_irq_ena(struct tfa_device *tfa, enum tfa9912_irq bit, int state)
{
	...
	if (bit == tfa9912_irq_all) {
		...
		for (reg = TFA98XX_INTERRUPT_ENABLE_REG1; reg <= TFA98XX_INTERRUPT_ENABLE_REG1 + tfa9912_irq_max / 16; reg++) {
			tfa_dsp_reg_write(tfa, (unsigned char)reg, state ? 0xffff : 0); /* all bits */
		...
		if (new_value != value) {
			tfa_dsp_reg_write(tfa, (unsigned char)reg, new_value); /* only this bit */
...
int tfa_irq_mask(struct tfa_device *tfa)
{
	...
	/* operate on all bits */
	for (reg = TFA98XX_INTERRUPT_ENABLE_REG1; reg <= TFA98XX_INTERRUPT_ENABLE_REG1 + tfa9912_irq_max / 16; reg++)
		tfa_dsp_reg_write(tfa, (unsigned char)reg, 0);
...
int tfa_irq_unmask(struct tfa_device *tfa)
{
	...
	/* operate on all bits */
	for (reg = TFA98XX_INTERRUPT_ENABLE_REG1; reg <= TFA98XX_INTERRUPT_ENABLE_REG1 + tfa9912_irq_max / 16; reg++)
		tfa_dsp_reg_write(tfa, (unsigned char)reg, tfa->interrupt_enable[reg - TFA98XX_INTERRUPT_ENABLE_REG1]);
...
int tfa987x_irq_enable(struct tfa_device *tfa, int bit, int status)
{
	...
	if (tfa2_irq_all == bit) {
		err = tfa_dsp_reg_write(tfa, TFA98XX_INTERRUPT_ENABLE_REG1, status ? 0xffff : 0); /* all bits */
		...
		err = tfa_dsp_reg_read(tfa, TFA98XX_INTERRUPT_ENABLE_REG1, &value);
		if (status)
		...
		if (new_value != value) {
			err = tfa_dsp_reg_write(tfa, TFA98XX_INTERRUPT_ENABLE_REG1, new_value);
...
void tfa987x_irq_mask(struct tfa_device *tfa)
{
	...
	err = tfa_dsp_reg_write(tfa, TFA98XX_INTERRUPT_ENABLE_REG1, 0);
}
...
void tfa987x_irq_unmask(struct tfa_device *tfa)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	pr_debug("entry [slave=0x%x]\n", tfa->slave_address);
	err = tfa_dsp_reg_write(tfa, TFA98XX_INTERRUPT_ENABLE_REG1, tfa->interrupt_enable[0]);
}
...
int tfa987x_irq_clear(struct tfa_device *tfa, int bit)
{
	...
	if (tfa2_irq_all == bit) {
		err = tfa_dsp_reg_write(tfa, TFA98XX_INTERRUPT_IN_REG1, 0xffff); /* all bits */
	} else {
		uint16_t value = 0, new_value = 0;
		if (((tfa->rev & 0xff) == 0x74) && (bit >= tfa9874_irq_max)) {
			pr_err("tfa9874 un-supported bit(0x%x)\n", bit);
		...
		err = tfa_dsp_reg_read(tfa, TFA98XX_INTERRUPT_IN_REG1, &value);
		new_value = value | (1<<bit);

		pr_debug("value=0x%x  new_value=0x%x\n", value, new_value);
		if (new_value != value) {
			err = tfa_dsp_reg_write(tfa, TFA98XX_INTERRUPT_IN_REG1, new_value);
		}
	}
...
int tfa987x_irq_handle(struct tfa_device *tfa)
{
	...
	err = tfa_dsp_reg_read(tfa, TFA98XX_INTERRUPT_OUT_REG1, &value);
	...
	err = tfa_dsp_reg_read(tfa, 0x10, &value);
...
int tfa98xx_powerswitch_is_enabled(struct tfa_device *tfa)
{
	...
	if (((tfa->rev & 0xff) == 0x13) || ((tfa->rev & 0xff) == 0x88)) {
		ret = tfa_dsp_reg_read(tfa, 0xc6, &value);
...
void tfa98xx_key2(struct tfa_device *tfa, int lock)
{
	...
	tfa_dsp_reg_write(tfa, (tfa->tfa_family == 1) ? 0x40 : 0x0F, 0x5A6B);
	/* lock/unlock key2 MTPK */
	TFA_WRITE_REG(tfa, MTPKEY2, lock ? 0 : 0x5A);
	/* unhide lock registers */
	if (!tfa->advance_keys_handling) /*artf65038*/
		tfa_dsp_reg_write(tfa, (tfa->tfa_family == 1) ? 0x40 : 0x0F, 0);
}
void tfa2_manual_mtp_cpy(struct tfa_device *tfa, uint16_t reg_row_to_keep, uint16_t reg_row_to_set, uint8_t row)///MCH_TO_TEST
{
	...
	tfa_dsp_reg_read(tfa, (unsigned char)reg_row_to_keep, &value);
	if (!row)
	{
		tfa_dsp_reg_write(tfa, 0xA7, value);
		tfa_dsp_reg_write(tfa, 0xA8, reg_row_to_set);
	}
	else
	{
		tfa_dsp_reg_write(tfa, 0xA7, reg_row_to_set);
		tfa_dsp_reg_write(tfa, 0xA8, value);
	}
	tfa_dsp_reg_write(tfa, 0xA3, 0x10 | row);
	if (tfa->is_probus_device)
	{
		/* Assure FAIM is enabled (enable it when neccesery) */
...
enum Tfa98xx_Error tfa_dsp_reg_read(struct tfa_device *tfa, unsigned char subaddress, unsigned short *value)
{
	enum Tfa98xx_Error error;
...
enum Tfa98xx_Error tfa_dsp_reg_write(struct tfa_device *tfa, unsigned char subaddress, unsigned short value)
{
	enum Tfa98xx_Error error;
...
int tfa_set_bf(struct tfa_device *tfa, const uint16_t bf, const uint16_t value)
{
	...
	err = tfa_dsp_reg_read(tfa, address, &regvalue);
	if (err) {
	...
	if (oldvalue != regvalue) {
		err = tfa_dsp_reg_write(tfa, address, regvalue);
		if (err) {
...
int tfa_write_reg(struct tfa_device *tfa, const uint16_t bf, const uint16_t reg_value)
{
	...
	err = tfa_dsp_reg_write(tfa, address, reg_value);
	if (err)
		return -err;
...
int tfa_dev_mtp_get(struct tfa_device *tfa, enum tfa_mtp item)
{
	...
	case TFA_MTP_RE25_PRIM:
		if (tfa->tfa_family == 2) {
			...
			tfa_dsp_reg_read(tfa, 0x83, (unsigned short*)&value);
		}
		break;
