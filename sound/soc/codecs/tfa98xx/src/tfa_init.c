...
static enum Tfa98xx_Error tfa9912_specific(struct tfa_device *tfa)
{
	...
	error = tfa_dsp_reg_write(tfa, 0x0F, 0x5A6B);
	error = tfa_dsp_reg_read(tfa, 0xFB, &value);
	xor = value ^ 0x005A;
	error = tfa_dsp_reg_write(tfa, 0xA0, xor);
	...
	if (tfa->rev == 0x1a13) {
		...
		/* -----  version 1.43  ----- */
		tfa_dsp_reg_write(tfa, 0x00, 0x0255); //POR=0x0245
		tfa_dsp_reg_write(tfa, 0x01, 0x838a); //POR=0x83ca
		tfa_dsp_reg_write(tfa, 0x02, 0x2dc8); //POR=0x2828
		tfa_dsp_reg_write(tfa, 0x05, 0x762a); //POR=0x766a
		tfa_dsp_reg_write(tfa, 0x22, 0x543c); //POR=0x545c
		tfa_dsp_reg_write(tfa, 0x26, 0x0100); //POR=0x0010
		tfa_dsp_reg_write(tfa, 0x51, 0x0000); //POR=0x0080
		tfa_dsp_reg_write(tfa, 0x52, 0x551c); //POR=0x1afc
		tfa_dsp_reg_write(tfa, 0x53, 0x003e); //POR=0x001e
		tfa_dsp_reg_write(tfa, 0x61, 0x000c); //POR=0x0018
		tfa_dsp_reg_write(tfa, 0x63, 0x0a96); //POR=0x0a9a
		tfa_dsp_reg_write(tfa, 0x65, 0x0a82); //POR=0x0a8b
		tfa_dsp_reg_write(tfa, 0x66, 0x0701); //POR=0x0700
		tfa_dsp_reg_write(tfa, 0x6c, 0x00d5); //POR=0x02d5
		tfa_dsp_reg_write(tfa, 0x70, 0x26f8); //POR=0x06e0
		tfa_dsp_reg_write(tfa, 0x71, 0x3074); //POR=0x2074
		tfa_dsp_reg_write(tfa, 0x75, 0x4484); //POR=0x4585
		tfa_dsp_reg_write(tfa, 0x76, 0x72ea); //POR=0x54a2
		tfa_dsp_reg_write(tfa, 0x83, 0x0716); //POR=0x0617
		tfa_dsp_reg_write(tfa, 0x89, 0x0013); //POR=0x0014
		tfa_dsp_reg_write(tfa, 0xb0, 0x4c08); //POR=0x4c00
		tfa_dsp_reg_write(tfa, 0xc6, 0x004e); //POR=0x000e /* PLMA5539: Please make sure bit 6 is always on! */
		/* ----- generated code end   ----- */
...
static enum Tfa98xx_Error tfa9912_auto_copy_mtp_to_iic(struct tfa_device *tfa)
{
	...
	return tfa_dsp_reg_write(tfa, 0xA3, 0x20);
}
...
static uint16_t tfa9912_vboost_fixup(struct tfa_device *tfa, uint16_t dcdc_cnt6)
{
	...
	tfa_dsp_reg_read(tfa, TFA9912_CALIBR_REG, &cal_offset);
...
static enum Tfa98xx_Error tfa9872_specific(struct tfa_device *tfa)
{
	...
	case 0x2a72:
		/* ----- generated code start ----- */
		/* -----  version 26 ----- */
		tfa_dsp_reg_write(tfa, 0x00, 0x1801); //POR=0x0001
		tfa_dsp_reg_write(tfa, 0x02, 0x2dc8); //POR=0x2028
		tfa_dsp_reg_write(tfa, 0x20, 0x0890); //POR=0x2890
		tfa_dsp_reg_write(tfa, 0x22, 0x043c); //POR=0x045c
		tfa_dsp_reg_write(tfa, 0x51, 0x0000); //POR=0x0080
		tfa_dsp_reg_write(tfa, 0x52, 0x1a1c); //POR=0x7ae8
		tfa_dsp_reg_write(tfa, 0x58, 0x161c); //POR=0x101c
		tfa_dsp_reg_write(tfa, 0x61, 0x0198); //POR=0x0000
		tfa_dsp_reg_write(tfa, 0x65, 0x0a8b); //POR=0x0a9a
		tfa_dsp_reg_write(tfa, 0x70, 0x07f5); //POR=0x06e6
		tfa_dsp_reg_write(tfa, 0x74, 0xcc84); //POR=0xd823
		tfa_dsp_reg_write(tfa, 0x82, 0x01ed); //POR=0x000d
		tfa_dsp_reg_write(tfa, 0x83, 0x0014); //POR=0x0013
		tfa_dsp_reg_write(tfa, 0x84, 0x0021); //POR=0x0020
		tfa_dsp_reg_write(tfa, 0x85, 0x0001); //POR=0x0003
		/* ----- generated code end   ----- */
		break;
	case 0x1b72:
	case 0x2b72:
	case 0x3b72:
		...
		tfa_dsp_reg_write(tfa, 0x20, 0x0890); //POR=0x2890
		tfa_dsp_reg_write(tfa, 0x22, 0x043c); //POR=0x045c
		tfa_dsp_reg_write(tfa, 0x23, 0x0001); //POR=0x0003
		tfa_dsp_reg_write(tfa, 0x51, 0x0000); //POR=0x0080
		tfa_dsp_reg_write(tfa, 0x52, 0x5a1c); //POR=0x7a08
		tfa_dsp_reg_write(tfa, 0x61, 0x0198); //POR=0x0000
		tfa_dsp_reg_write(tfa, 0x63, 0x0a9a); //POR=0x0a93
		tfa_dsp_reg_write(tfa, 0x65, 0x0a82); //POR=0x0a8d
		tfa_dsp_reg_write(tfa, 0x6f, 0x01e3); //POR=0x02e4
		tfa_dsp_reg_write(tfa, 0x70, 0x06fd); //POR=0x06e6
		tfa_dsp_reg_write(tfa, 0x71, 0x307e); //POR=0x207e
		tfa_dsp_reg_write(tfa, 0x74, 0xcc84); //POR=0xd913
		tfa_dsp_reg_write(tfa, 0x75, 0x1132); //POR=0x118a
		tfa_dsp_reg_write(tfa, 0x82, 0x01ed); //POR=0x000d
		tfa_dsp_reg_write(tfa, 0x83, 0x001a); //POR=0x0013
		/* ----- generated code end   ----- */
		break;
	default:
	...
	error = tfa_dsp_reg_read(tfa, 0xB0, &value);
	value |= 1 << 3;
	error = tfa_dsp_reg_write(tfa, 0xB0, value);
...
static enum Tfa98xx_Error tfa9873_specific(struct tfa_device* tfa)
{
	...
	case 0x0a73:
		...
		/* ----- generated code start ----- */
		/* -----  version 28 ----- */
		tfa_dsp_reg_write(tfa, 0x02, 0x0628); //POR=0x0008
		tfa_dsp_reg_write(tfa, 0x4c, 0x00e9); //POR=0x00ff
		tfa_dsp_reg_write(tfa, 0x52, 0x17d0); //POR=0x57d0
		tfa_dsp_reg_write(tfa, 0x56, 0x0011); //POR=0x0019
		tfa_dsp_reg_write(tfa, 0x58, 0x0200); //POR=0x0210
		tfa_dsp_reg_write(tfa, 0x59, 0x0001); //POR=0x0000
		tfa_dsp_reg_write(tfa, 0x5f, 0x0180); //POR=0x0100
		tfa_dsp_reg_write(tfa, 0x61, 0x0183); //POR=0x0a82
		tfa_dsp_reg_write(tfa, 0x63, 0x055a); //POR=0x0a9a
		tfa_dsp_reg_write(tfa, 0x65, 0x0542); //POR=0x0a82
		tfa_dsp_reg_write(tfa, 0x6f, 0x00a3); //POR=0x0085
		tfa_dsp_reg_write(tfa, 0x70, 0xa3fb); //POR=0x23fb
		tfa_dsp_reg_write(tfa, 0x71, 0x007e); //POR=0x107e
		tfa_dsp_reg_write(tfa, 0x83, 0x009a); //POR=0x0799
		tfa_dsp_reg_write(tfa, 0x84, 0x0211); //POR=0x0011
		tfa_dsp_reg_write(tfa, 0x85, 0x0382); //POR=0x0380
		tfa_dsp_reg_write(tfa, 0x8c, 0x0210); //POR=0x0010
		tfa_dsp_reg_write(tfa, 0xd5, 0x0000); //POR=0x0100
		/* ----- generated code end   ----- */
		break;
	case 0x0b73:
		/* ----- generated code start ----- */
		/* -----  version 13 ----- */
		tfa_dsp_reg_write(tfa, 0x02, 0x0628); //POR=0x0008
		tfa_dsp_reg_write(tfa, 0x61, 0x0183); //POR=0x0182
		tfa_dsp_reg_write(tfa, 0x63, 0x005a); //POR=0x055a
		tfa_dsp_reg_write(tfa, 0x6f, 0x0083); //POR=0x00a5
		tfa_dsp_reg_write(tfa, 0x70, 0xa3fb); //POR=0x23fb
		tfa_dsp_reg_write(tfa, 0x73, 0x0187); //POR=0x0107
		tfa_dsp_reg_write(tfa, 0x83, 0x009a); //POR=0x0799
		tfa_dsp_reg_write(tfa, 0x85, 0x0380); //POR=0x0382
		tfa_dsp_reg_write(tfa, 0xd5, 0x004d); //POR=0x014d
		/* ----- generated code end   ----- */
		break;
	case 0x1a73:
		break;
...
static enum Tfa98xx_Error tfa9874_specific(struct tfa_device *tfa)
{
	...
	case 0x0a74: /* Initial revision ID */
				 /* ----- generated code start ----- */
				 /* V25 */
		tfa_dsp_reg_write(tfa, 0x02, 0x22a8); //POR=0x25c8
		tfa_dsp_reg_write(tfa, 0x51, 0x0020); //POR=0x0000
		tfa_dsp_reg_write(tfa, 0x52, 0x57dc); //POR=0x56dc
		tfa_dsp_reg_write(tfa, 0x58, 0x16a4); //POR=0x1614
		tfa_dsp_reg_write(tfa, 0x61, 0x0110); //POR=0x0198
		tfa_dsp_reg_write(tfa, 0x66, 0x0701); //POR=0x0700
		tfa_dsp_reg_write(tfa, 0x6f, 0x00a3); //POR=0x01a3
		tfa_dsp_reg_write(tfa, 0x70, 0x07f8); //POR=0x06f8
		tfa_dsp_reg_write(tfa, 0x73, 0x0007); //POR=0x0005
		tfa_dsp_reg_write(tfa, 0x74, 0x5068); //POR=0xcc80
		tfa_dsp_reg_write(tfa, 0x75, 0x0d28); //POR=0x1138
		tfa_dsp_reg_write(tfa, 0x83, 0x0594); //POR=0x061a
		tfa_dsp_reg_write(tfa, 0x84, 0x0001); //POR=0x0021
		tfa_dsp_reg_write(tfa, 0x85, 0x0001); //POR=0x0003
		tfa_dsp_reg_write(tfa, 0x88, 0x0000); //POR=0x0002
		tfa_dsp_reg_write(tfa, 0xc4, 0x2001); //POR=0x0001
									  /* ----- generated code end   ----- */
		break;
	case 0x0b74:
		/* ----- generated code start ----- */
		...
		tfa_dsp_reg_write(tfa, 0x6f, 0x00a3); //POR=0x01a3
		tfa_dsp_reg_write(tfa, 0x70, 0x07f8); //POR=0x06f8
		tfa_dsp_reg_write(tfa, 0x73, 0x0047); //POR=0x0045
		tfa_dsp_reg_write(tfa, 0x74, 0x5068); //POR=0xcc80
		tfa_dsp_reg_write(tfa, 0x75, 0x0d28); //POR=0x1138
		tfa_dsp_reg_write(tfa, 0x83, 0x0595); //POR=0x061a
		tfa_dsp_reg_write(tfa, 0x84, 0x0001); //POR=0x0021
		tfa_dsp_reg_write(tfa, 0x85, 0x0001); //POR=0x0003
		tfa_dsp_reg_write(tfa, 0x88, 0x0000); //POR=0x0002
		tfa_dsp_reg_write(tfa, 0xc4, 0x2001); //POR=0x0001
	...
	case 0x0c74:
		/* ----- generated code start ----- */
		/* V1.16 */
		tfa_dsp_reg_write(tfa, 0x02, 0x22c8); //POR=0x25c8
		tfa_dsp_reg_write(tfa, 0x52, 0x57dc); //POR=0x56dc
		tfa_dsp_reg_write(tfa, 0x53, 0x003e); //POR=0x001e
		tfa_dsp_reg_write(tfa, 0x56, 0x0400); //POR=0x0600
		tfa_dsp_reg_write(tfa, 0x61, 0x0110); //POR=0x0198
		tfa_dsp_reg_write(tfa, 0x6f, 0x00a5); //POR=0x01a3
		tfa_dsp_reg_write(tfa, 0x70, 0x07f8); //POR=0x06f8
		tfa_dsp_reg_write(tfa, 0x73, 0x0047); //POR=0x0045
		tfa_dsp_reg_write(tfa, 0x74, 0x5098); //POR=0xcc80
		tfa_dsp_reg_write(tfa, 0x75, 0x8d28); //POR=0x1138
		tfa_dsp_reg_write(tfa, 0x80, 0x0000); //POR=0x0003
		tfa_dsp_reg_write(tfa, 0x83, 0x0799); //POR=0x061a
		tfa_dsp_reg_write(tfa, 0x84, 0x0081); //POR=0x0021
									  /* ----- generated code end   ----- */
		break;
	default:
		pr_info("\nWarning: Optimal settings not found for device with revid = 0x%x \n", tfa->rev);
...
/* ----- generated code start ----- */
/* -----  version 28 ----- */
		tfa_dsp_reg_write(tfa, 0x01, 0x2e18); //POR=0x2e88
		tfa_dsp_reg_write(tfa, 0x02, 0x0628); //POR=0x0008
		tfa_dsp_reg_write(tfa, 0x04, 0x0240); //POR=0x0340
		tfa_dsp_reg_write(tfa, 0x52, 0x587c); //POR=0x57dc
		tfa_dsp_reg_write(tfa, 0x61, 0x0183); //POR=0x0a82
		tfa_dsp_reg_write(tfa, 0x63, 0x055a); //POR=0x0a9a
		tfa_dsp_reg_write(tfa, 0x65, 0x0542); //POR=0x0a82
		tfa_dsp_reg_write(tfa, 0x71, 0x303e); //POR=0x307e
		tfa_dsp_reg_write(tfa, 0x83, 0x009a); //POR=0x0799
		/* ----- generated code end   ----- */
	...
	case 0x1a78: /* Initial revision ID */
		...
		tfa_dsp_reg_write(tfa, 0x01, 0x2e18); //POR=0x2e88
		tfa_dsp_reg_write(tfa, 0x02, 0x0628); //POR=0x0008
		tfa_dsp_reg_write(tfa, 0x04, 0x0241); //POR=0x0340
		tfa_dsp_reg_write(tfa, 0x52, 0x587c); //POR=0x57dc
		tfa_dsp_reg_write(tfa, 0x61, 0x0183); //POR=0x0a82
		tfa_dsp_reg_write(tfa, 0x63, 0x055a); //POR=0x0a9a
		tfa_dsp_reg_write(tfa, 0x65, 0x0542); //POR=0x0a82
		tfa_dsp_reg_write(tfa, 0x70, 0xb7ff); //POR=0x37ff
		tfa_dsp_reg_write(tfa, 0x71, 0x303e); //POR=0x307e
		tfa_dsp_reg_write(tfa, 0x83, 0x009a); //POR=0x0799
		tfa_dsp_reg_write(tfa, 0x84, 0x0211); //POR=0x0011
		tfa_dsp_reg_write(tfa, 0x8c, 0x0210); //POR=0x0010
		tfa_dsp_reg_write(tfa, 0xce, 0x2202); //POR=0xa202
		tfa_dsp_reg_write(tfa, 0xd5, 0x0000); //POR=0x0100
		/* ----- generated code end   ----- */
...
static enum Tfa98xx_Error tfa9888_specific(struct tfa_device *tfa)
{
	...
	/* --------- Version v1 ---------- */
	if (tfa->rev == 0x2c88) {
		tfa_dsp_reg_write(tfa, 0x00, 0x164d); //POR=0x064d
		tfa_dsp_reg_write(tfa, 0x01, 0x828b); //POR=0x92cb
		tfa_dsp_reg_write(tfa, 0x02, 0x1dc8); //POR=0x1828
		tfa_dsp_reg_write(tfa, 0x0e, 0x0080); //POR=0x0000
		tfa_dsp_reg_write(tfa, 0x20, 0x089e); //POR=0x0890
		tfa_dsp_reg_write(tfa, 0x22, 0x543c); //POR=0x545c
		tfa_dsp_reg_write(tfa, 0x23, 0x0006); //POR=0x0000
		tfa_dsp_reg_write(tfa, 0x24, 0x0014); //POR=0x0000
		tfa_dsp_reg_write(tfa, 0x25, 0x000a); //POR=0x0000
		tfa_dsp_reg_write(tfa, 0x26, 0x0100); //POR=0x0000
		tfa_dsp_reg_write(tfa, 0x28, 0x1000); //POR=0x0000
		tfa_dsp_reg_write(tfa, 0x51, 0x0000); //POR=0x00c0
		tfa_dsp_reg_write(tfa, 0x52, 0xfafe); //POR=0xbaf6
		tfa_dsp_reg_write(tfa, 0x70, 0x3ee4); //POR=0x3ee6
		tfa_dsp_reg_write(tfa, 0x71, 0x1074); //POR=0x3074
		tfa_dsp_reg_write(tfa, 0x83, 0x0014); //POR=0x0013
		/* ----- generated code end   ----- */
	}
	else {
...
static enum Tfa98xx_Error tfa9896_specific(struct tfa_device *tfa)
{
	...
	*/
	if (tfa->rev == 0x1b96) {
		...
		tfa_dsp_reg_write(tfa, 0x06, 0x000b); //POR=0x0001
		tfa_dsp_reg_write(tfa, 0x07, 0x3e7f); //POR=0x1e7f
		tfa_dsp_reg_write(tfa, 0x0a, 0x0d8a); //POR=0x0592
		tfa_dsp_reg_write(tfa, 0x48, 0x0300); //POR=0x0308
		tfa_dsp_reg_write(tfa, 0x88, 0x0100); //POR=0x0000
		/* ----- generated code end   ----- */
	}
	...
	error = tfa_dsp_reg_read(tfa, 0x49, &check_value);
	check_value &= ~0x1;
	error = tfa_dsp_reg_write(tfa, 0x49, check_value);
	return error;
}
...
static enum Tfa98xx_Error tfa9897_specific(struct tfa_device *tfa)
{
	...
	* When Iddqtestbst is set to "0", the slewrate is reduced.
	* This will lower the overshoot on IN-B to avoid NMOS damage of booster */
	error = tfa_dsp_reg_write(tfa, 0x48, 0x0300); /* POR value = 0x308 */
...
static enum Tfa98xx_Error tfa9895_specific(struct tfa_device *tfa)
{
	...
	tfa_dsp_reg_write(tfa, 0x05, 0x13AB);
	tfa_dsp_reg_write(tfa, 0x06, 0x001F);
	/* peak voltage protection is always on, but may be written */
	tfa_dsp_reg_write(tfa, 0x08, 0x3C4E);
	/*TFA98XX_SYSCTRL_DCA=0*/
	tfa_dsp_reg_write(tfa, 0x09, 0x024D);
	tfa_dsp_reg_write(tfa, 0x41, 0x0308);
	error = tfa_dsp_reg_write(tfa, 0x49, 0x0E82);
...
static enum Tfa98xx_Error tfa9891_specific(struct tfa_device *tfa)
{
	...
	/* ----- generated code start ----- */
	/* -----  version 18.0 ----- */
	tfa_dsp_reg_write(tfa, 0x09, 0x025d); //POR=0x024d
	tfa_dsp_reg_write(tfa, 0x10, 0x0018); //POR=0x0024
	tfa_dsp_reg_write(tfa, 0x22, 0x0003); //POR=0x0023
	tfa_dsp_reg_write(tfa, 0x25, 0x0001); //POR=0x0000
	tfa_dsp_reg_write(tfa, 0x46, 0x0000); //POR=0x4000
	tfa_dsp_reg_write(tfa, 0x55, 0x3ffb); //POR=0x7fff
	/* ----- generated code end   ----- */
...
static enum Tfa98xx_Error tfa9890_specific(struct tfa_device *tfa)
{
	...
	error = tfa_dsp_reg_write(tfa, 0x40, 0x5a6b);
	if (error)
		return error;
	tfa_dsp_reg_read(tfa, 0x59, &regRead);
	regRead |= 0x3;
	tfa_dsp_reg_write(tfa, 0x59, regRead);
	error = tfa_dsp_reg_write(tfa, 0x40, 0x0000);
	error = tfa_dsp_reg_write(tfa, 0x47, 0x7BE1);
...
static enum Tfa98xx_Error tfa9890_clockgating(struct tfa_device *tfa, int on)
{
	...
	error = tfa_dsp_reg_read(tfa, TFA98XX_CURRENTSENSE4, &value);
	...
	if (Tfa98xx_Error_Ok == error) {
		...
		error = tfa_dsp_reg_write(tfa, TFA98XX_CURRENTSENSE4, value);
	}
...
static enum Tfa98xx_Error tfa9890_dsp_system_stable(struct tfa_device *tfa, int *ready)
{
	...
	*ready = !((TFA_GET_BF_VALUE(tfa, AREFS, status) == 0)
	...
	error = tfa_dsp_reg_read(tfa, 0x84, &mtp0);
	if (error)
		goto errorExit;
...
static enum Tfa98xx_Error tfa9894_specific(struct tfa_device *tfa)
{
	...
	if (tfa->rev == 0x0a94) {
		/* V36 */
		/* ----- generated code start ----- */
		tfa_dsp_reg_write(tfa, 0x00, 0xa245); //POR=0x8245
		tfa_dsp_reg_write(tfa, 0x02, 0x51e8); //POR=0x55c8
		tfa_dsp_reg_write(tfa, 0x52, 0xbe17); //POR=0xb617
		tfa_dsp_reg_write(tfa, 0x57, 0x0344); //POR=0x0366
		tfa_dsp_reg_write(tfa, 0x61, 0x0033); //POR=0x0073
		tfa_dsp_reg_write(tfa, 0x71, 0x00cf); //POR=0x018d
		tfa_dsp_reg_write(tfa, 0x72, 0x34a9); //POR=0x44e8
		tfa_dsp_reg_write(tfa, 0x73, 0x3808); //POR=0x3806
		tfa_dsp_reg_write(tfa, 0x76, 0x0067); //POR=0x0065
		tfa_dsp_reg_write(tfa, 0x80, 0x0000); //POR=0x0003
		tfa_dsp_reg_write(tfa, 0x81, 0x5715); //POR=0x561a
		tfa_dsp_reg_write(tfa, 0x82, 0x0104); //POR=0x0044
		/* ----- generated code end   ----- */
	}
		...
		/* ----- generated code start ----- */
		tfa_dsp_reg_write(tfa, 0x00, 0xa245); //POR=0x8245
		tfa_dsp_reg_write(tfa, 0x01, 0x15da); //POR=0x11ca
		tfa_dsp_reg_write(tfa, 0x02, 0x5288); //POR=0x55c8
		tfa_dsp_reg_write(tfa, 0x52, 0xbe17); //POR=0xb617
		tfa_dsp_reg_write(tfa, 0x53, 0x0dbe); //POR=0x0d9e
		tfa_dsp_reg_write(tfa, 0x56, 0x05c3); //POR=0x07c3
		tfa_dsp_reg_write(tfa, 0x57, 0x0344); //POR=0x0366
		tfa_dsp_reg_write(tfa, 0x61, 0x0032); //POR=0x0073
		tfa_dsp_reg_write(tfa, 0x71, 0x00cf); //POR=0x018d
		tfa_dsp_reg_write(tfa, 0x72, 0x34a9); //POR=0x44e8
		tfa_dsp_reg_write(tfa, 0x73, 0x38c8); //POR=0x3806
		tfa_dsp_reg_write(tfa, 0x76, 0x0067); //POR=0x0065
		tfa_dsp_reg_write(tfa, 0x80, 0x0000); //POR=0x0003
		tfa_dsp_reg_write(tfa, 0x81, 0x5799); //POR=0x561a
		tfa_dsp_reg_write(tfa, 0x82, 0x0104); //POR=0x0044
		/* ----- generated code end ----- */
		...
		tfa_dsp_reg_write(tfa, 0x01, 0x15da); //POR=0x11ca
		tfa_dsp_reg_write(tfa, 0x02, 0x51e8); //POR=0x55c8
		tfa_dsp_reg_write(tfa, 0x04, 0x0200); //POR=0x0000
		tfa_dsp_reg_write(tfa, 0x52, 0xbe17); //POR=0xb617
		tfa_dsp_reg_write(tfa, 0x53, 0x0dbe); //POR=0x0d9e
		tfa_dsp_reg_write(tfa, 0x57, 0x0344); //POR=0x0366
		tfa_dsp_reg_write(tfa, 0x61, 0x0032); //POR=0x0073
		tfa_dsp_reg_write(tfa, 0x71, 0x6ecf); //POR=0x6f8d
		tfa_dsp_reg_write(tfa, 0x72, 0xb4a9); //POR=0x44e8
		tfa_dsp_reg_write(tfa, 0x73, 0x38c8); //POR=0x3806
		tfa_dsp_reg_write(tfa, 0x76, 0x0067); //POR=0x0065
		tfa_dsp_reg_write(tfa, 0x80, 0x0000); //POR=0x0003
		tfa_dsp_reg_write(tfa, 0x81, 0x5799); //POR=0x561a
