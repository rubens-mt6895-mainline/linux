...
enum Tfa98xx_Error tfaRunReadBitfield(struct tfa_device *tfa, TfaBitfield_t *bf)
{
	...
	error = tfa_dsp_reg_read(tfa, (unsigned char)(bfUni.Enum.address), &regvalue);
...
static enum Tfa98xx_Error tfaRunWriteRegister(struct tfa_device *tfa, TfaRegpatch_t *reg)
{
	...
	error = tfa_dsp_reg_read(tfa, reg->address, &value);
	if (error) return error;
	...
	value |= newvalue;
	error = tfa_dsp_reg_write(tfa, reg->address, value);
